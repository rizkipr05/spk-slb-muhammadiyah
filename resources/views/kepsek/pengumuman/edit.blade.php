...
@section('title', 'Edit Pengumuman - SPK Disabilitas')
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <span>Form Edit Pengumuman</span>
                <a href="{{ route('kepsek.pengumuman.index') }}" class="btn btn-sm btn-light border">
                    <i class="bi bi-arrow-left"></i> Kembali
                </a>
            </div>
            <div class="card-body">
                <form action="{{ route('kepsek.pengumuman.update', $pengumuman->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('isi') is-invalid @enderror" id="isi" name="isi" rows="6" required placeholder="Tuliskan isi pengumuman secara detail...">{{ old('isi', $pengumuman->isi) }}</textarea>
                        @error('isi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4 form-check form-switch">
                        <input class="form-check-input" type="checkbox" role="switch" id="status_aktif" name="status_aktif" value="1" {{ old('status_aktif', $pengumuman->status_aktif) ? 'checked' : '' }}>
